...
模板里写监理 SA 禁止做 3 件事：
- ❌ 拉其他 subagent（**foreman 角色专属限制**：数据源 SA 由主会话管）
- ❌ 读原始数据内容（只读 progress.json，省 token）
- ❌ 修改子任务的 pageId 或文件（避免冲突）

> ⚠️ **重要区分**：第 1 条"❌ 拉其他 subagent"是 **foreman 角色的专属限制**（因为数据源 SA 由主会话直接管，foreman 不需要再拉下层）。**其他非 foreman 的第 1 层 SA 不受此限制**——它们按"两层嵌套规则"可以拉下层，只是强制用最省 token 模型。
>
> 这两个概念曾被同一种表述"❌ 拉起其他 subagent（两层嵌套强制用最省 token 模型）"混淆，导致 AI 错误地把 foreman 专属限制泛化到所有第 1 层 SA。模板已修正为明确区分。
